...
import Data.Semigroup (Max (..), Min (..), Semigroup (..), Sum (..), sconcat)
...
parseProductName s = if not $ null s then Just s else Nothing

parseTradeType :: String -> Maybe TradeType
parseTradeType = readMaybe . strip

parseCost :: String -> Maybe Int
parseCost = mfilter (>= 0) . readMaybe . strip

parseRow :: String -> Maybe Row
parseRow s = case splitOn "," s of
  [productName, tradeTypeStr, costStr] -> do
    productName' <- parseProductName productName
    tradeType <- parseTradeType tradeTypeStr
    cost <- parseCost costStr
    Just $ Row productName' tradeType cost
  _ -> Nothing
...
rowToStats (Row productName Buy cost) =
  Stats
    { statsTotalPositions = Sum 1,
      ...
      statsLongest = MaxLen productName
    }
rowToStats (Row productName Sell cost) =
  Stats
    { statsTotalPositions = Sum 1,
      statsTotalSum = Sum cost,
      statsAbsoluteMax = Max cost,
...
calculateStats input = case nonEmpty validRows of
  Just rows -> displayStats $ combineRows rows
  Nothing -> "The files doesn't have any products."
  where
    validRows = mapMaybe parseRow $ drop 1 $ lines input
...
main = do
  args <- getArgs 
  case args of
    [fp] -> printProductStats fp
    _ -> putStrLn "Missing filepath."
