...
lowerAndGreater n = countLowerAndGreater 0 0
  where
    countLowerAndGreater :: Int -> Int -> [Int] -> String
    countLowerAndGreater lower greater xs
      | null xs = formatOutput lower greater
      | head xs > n = countLowerAndGreater (lower + 1) greater (tail xs)
      | head xs < n = countLowerAndGreater lower (greater + 1) (tail xs)
      | otherwise = countLowerAndGreater lower greater (tail xs)

    formatOutput :: Int -> Int -> String
    formatOutput lower greater =
      show n
        ++ " is greater than "
        ++ show greater
